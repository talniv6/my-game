...
{
    static float getCursorToModelX(float screenX, float cursorX)
    {
        return ((cursorX) * Game.V_WIDTH) / (screenX);
    }

    static float getCursorToModelY(float screenY, float cursorY)
    {
        return ((screenY - cursorY)) * Game.V_HEIGHT / (screenY) ;
    }
}
